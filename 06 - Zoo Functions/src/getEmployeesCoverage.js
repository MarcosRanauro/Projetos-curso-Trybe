...
const getSpecies = (name) => {
  ...
  const specieResponsible = species.filter(({ id }) => employeeResponsible.includes(id));

  return specieResponsible;
};
...
const getInfoEmployees = (name) => {
  ...
  const getResponsible = getSpecies(getEmployee);

  return {
    id: getEmployee.id,
    fullName: `${getEmployee.firstName} ${getEmployee.lastName}`,
    species: getSpeciesName(getResponsible),
    locations: getSpeciesLocation(getResponsible),
  };
};
